...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        self,
        policy: Union[str, Type[ActorCriticPolicy]], # replace it with RMA policy
    ...
    def collect_rollouts(
        ...
        """
        Collect experiences using the current policy and fill a ``Ph1RolloutBuffer``.
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                actions, values, log_probs = self.policy.forward(obs_tensor) # !replace with policy RMA has 
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            ...
            for ep_info in self.ep_info_buffer:
                reward_metrics = defaultdict(int)  
                for i in range(self.env.rew_dim - 1):
                    reward_metrics[ "rewards/{0}".format(self.env.reward_names[i])]+=ep_info[self.env.reward_names[i]]
                reward_metrics[ "rewards/total_reward"]+=ep_info["r"] 
                wandb.log({**reward_metrics, **{"rollout/ep_len_mean":ep_info["l"]}},step = self.num_timesteps)
                
            # if iteration % 5000000 == 0:
                # env_cfg["rewards"]["updating_oscillate_coeff"] = env_cfg["rewards"]["updating_oscillate_coeff"]*2
            ...
            if log_interval is not None and iteration % log_interval[1] == 0:
                ...
                if ep_rew_mean > self.best_reward:
                    ...
                    best_iter = 888
                    self.policy.save(policy_path + "/iter_{0:05d}.pth".format(best_iter))
                    self.env.save_rms(
