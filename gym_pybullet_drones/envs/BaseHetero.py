...
class BaseHetero(gym.Env):
    ...
    def __init__(self,
        ...
        if self.VISION_ATTR:
            ...
            if self.IMG_CAPTURE_FREQ%self.PYB_STEPS_PER_CTRL != 0:
                print("[ERROR] in BaseHetero.__init__(), PyBullet and control frequencies incompatible with the desired video capture frame rate ({:f}Hz)".format(self.IMG_FRAME_PER_SEC))
                exit()
            if self.RECORD:
        ...
        if initial_xyzs is None:
            self.INIT_XYZS = np.vstack([np.array([drone_idx1*4*drone_entity.L for drone_idx1, drone_entity in enumerate(self.DRONE_CFG)]), \
                                        np.array([drone_idx2*4*drone_entity.L for drone_idx2, drone_entity in enumerate(self.DRONE_CFG)]), \
                                        np.array([drone_entity.COLLISION_H/2-drone_entity.COLLISION_Z_OFFSET+.1 for drone_idx3, drone_entity in enumerate(self.DRONE_CFG)])]).transpose().reshape(self.NUM_DRONES, 3)
        
        elif np.array(initial_xyzs).shape == (self.NUM_DRONES, 3):
            self.INIT_XYZS = initial_xyzs
        else:
            print("[ERROR] invalid initial_xyzs in BaseHetero.__init__(), try initial_xyzs.reshape(NUM_DRONES,3)")
        if initial_rpys is None:
            self.INIT_RPYS = np.zeros((self.NUM_DRONES, 3))
        elif np.array(initial_rpys).shape == (self.NUM_DRONES, 3):
            self.INIT_RPYS = initial_rpys
        else:
            print("[ERROR] invalid initial_rpys in BaseHetero.__init__(), try initial_rpys.reshape(NUM_DRONES,3)")
    ...
    def render(self,
        ...
        if self.first_render_call and not self.GUI:
            print("[WARNING] BaseHetero.render() is implemented as text-only, re-initialize the environment using Aviary(gui=True) to use PyBullet's graphical interface")
            self.first_render_call = False
        print("\n[INFO] BaseHetero.render() ——— it {:04d}".format(self.step_counter),
        ...
        for i in range (self.NUM_DRONES):
            print("[INFO] BaseHetero.render() ——— drone {:d}".format(i),
    ...
    def _housekeeping(self):
        ...
        self.DRONE_IDS = np.array([p.loadURDF(pkg_resources.resource_filename('gym_pybullet_drones', 'assets/'+ drone_entity.URDF),
                                              self.INIT_XYZS[i,:],
                                              p.getQuaternionFromEuler(self.INIT_RPYS[i,:]),
                                              flags = p.URDF_USE_INERTIA_FROM_FILE,
                                              physicsClientId=self.CLIENT
                                              ) for i, drone_entity in enumerate(self.DRONE_CFG)])
    ...
    def _getDroneImages(self,
        ...
        if self.IMG_RES is None:
            print("[ERROR] in BaseHetero._getDroneImages(), remember to set self.IMG_RES to np.array([width, height])")
            exit()
    ...
    def _exportImage(self,
        ...
        if img_type == ImageType.RGB:
            ...
            print("[ERROR] in BaseHetero._exportImage(), unknown ImageType")
            exit()
        if img_type != ImageType.RGB:
    ...
    def _physics(self,
        ...
        """
        forces = np.array(rpm**2)*self.DRONE_CFG[nth_drone].KF
        torques = np.array(rpm**2)*self.DRONE_CFG[nth_drone].KM
        #================= Update: Don't Use RACE Model ======================
    ...
    def _groundEffect(self,
        ...
        prop_heights = np.clip(prop_heights, drone_entity.GND_EFF_H_CLIP, np.inf)
        gnd_effects = np.array(rpm**2) * drone_entity.KF * drone_entity.GND_EFF_COEFF * (drone_entity.PROP_RADIUS/(4 * prop_heights))**2
    ...
    def _downwash(self,
        ...
        for i in range(self.NUM_DRONES):
            ...
            if delta_z > 0 and delta_xy < 10: # Ignore drones more than 10 meters away
                alpha = drone_entity.DW_COEFF_1 * (drone_entity.PROP_RADIUS/(4*delta_z))**2
                beta = drone_entity.DW_COEFF_2 * delta_z + drone_entity.DW_COEFF_3
    ...
    def _dynamics(self,
        ...
        forces = np.array(rpm**2) * drone_entity.KF
        thrust = np.array([0, 0, np.sum(forces)])
        thrust_world_frame = np.dot(rotation, thrust)
        force_world_frame = thrust_world_frame - np.array([0, 0, drone_entity.GRAVITY])
        z_torques = np.array(rpm**2)*drone_entity.KM
        if drone_entity.DRONE_MODEL == DroneModel.RACE:
            z_torques = -z_torques
        z_torque = (-z_torques[0] + z_torques[1] - z_torques[2] + z_torques[3])
        if drone_entity.DRONE_MODEL==DroneModel.CF2X or drone_entity.DRONE_MODEL==DroneModel.RACE:
            x_torque = (forces[0] + forces[1] - forces[2] - forces[3]) * (drone_entity.L/np.sqrt(2))
            y_torque = (- forces[0] + forces[1] + forces[2] - forces[3]) * (drone_entity.L/np.sqrt(2))
        elif drone_entity.DRONE_MODEL==DroneModel.CF2P:
            x_torque = (forces[1] - forces[3]) * drone_entity.L
            y_torque = (-forces[0] + forces[2]) * drone_entity.L
        torques = np.array([x_torque, y_torque, z_torque])
        torques = torques - np.cross(rpy_rates, np.dot(drone_entity.J, rpy_rates))
        rpy_rates_deriv = np.dot(drone_entity.J_INV, torques)
        no_pybullet_dyn_accs = force_world_frame / drone_entity.M
        #==================== Update state =================================
    ...
    def _normalizedActionToRPM(self,
        ...
        if np.any(np.abs(action) > 1):
            print("\n[ERROR] it", self.step_counter, "in BaseHetero._normalizedActionToRPM(), out-of-bound action")
